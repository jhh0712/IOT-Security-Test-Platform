...
void CDB_Check::ThreadFunc()
{
	...
	while ((sql_row = mysql_fetch_row(sql_result)) != NULL)	//쿼리 실행결과 값 가져옴
	{
		...
		query_state2 = mysql_query(connection, "SELECT * FROM encryptkey WHERE date LIKE '" + date + "%'");	//date 값 이용해 RSA 암호화된 키값 선택 쿼리 실행

		sql_result2 = mysql_store_result(connection);	//쿼리 실행결과 저장
